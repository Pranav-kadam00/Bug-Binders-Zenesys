...
function Comparison({ twin = false }: { twin?: boolean }) {
  ...
  const id = Number(purchaseRequestId);
  const q = twin ? useGetDecisionTwin(id, { query: { enabled: !!id, queryKey: getGetDecisionTwinQueryKey(id) } }) : useGetVendorComparison(id, { query: { enabled: !!id, queryKey: getGetVendorComparisonQueryKey(id) } });
  
  const [negotiating, setNegotiating] = useState<number | null>(null);
  const [negResult, setNegResult] = useState<any>(null);

  const handleNegotiate = async (vendor: any) => {
    setNegotiating(vendor.vendorId);
    try {
      const price = vendor.quotedPrice || vendor.purchaseCost;
      const res = await fetch(`/api/v1/vendors/${vendor.vendorId}/negotiate`, {
        method: "POST", 
        headers: { 
          "Content-Type": "application/json",
          "Authorization": `Bearer ${localStorage.getItem("aqura_token") || ""}`
        },
        body: JSON.stringify({ productName: "Infrastructure Supplies", quantity: 1, currentPrice: price })
      });
      const data = await res.json();
      setNegResult({ ...data, vendorName: vendor.vendorName });
    } finally {
      setNegotiating(null);
    }
  };
          ...
          <div className="w-full max-w-2xl overflow-hidden rounded-2xl border border-border bg-background shadow-2xl">
            <div className="flex items-center justify-between border-b border-border bg-sidebar px-6 py-4 text-white">
              <div className="flex items-center gap-3">
                <div className="grid h-8 w-8 place-items-center rounded-lg bg-primary text-primary-foreground"><Sparkles size={16} /></div>
                <div>
                  <h3 className="font-serif text-lg font-bold">AI Agent Negotiation</h3>
                  <p className="text-[11px] text-sidebar-foreground/70">Negotiating with {negResult.vendorName}</p>
                </div>
              </div>
              <button onClick={() => setNegResult(null)} className="text-white/70 hover:text-white"><X size={18} /></button>
            </div>
            
            {!negResult.thread ? (
              <div className="p-6 text-center text-sm text-destructive">
                <p>Failed to simulate negotiation: {negResult.detail || "Unknown error occurred"}</p>
              </div>
            ) : (
              <div className="aqura-scrollbar max-h-[50vh] space-y-4 overflow-y-auto p-6">
                {negResult.thread.map((msg: any, i: number) => (
                  <div key={i} className={`flex ${msg.sender === "AQURA AI" ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-[85%] rounded-xl p-4 text-sm leading-6 shadow-sm ${msg.sender === "AQURA AI" ? "bg-primary text-primary-foreground" : "border border-border bg-card"}`}>
                      <p className={`mb-1 text-[10px] font-bold uppercase tracking-wider ${msg.sender === "AQURA AI" ? "text-primary-foreground/70" : "text-muted-foreground"}`}>{msg.sender}</p>
                      {msg.message}
                    </div>
                  </div>
                ))}
            ...
            <div className="border-t border-border bg-muted/30 p-6">
              {negResult.success && negResult.thread ? (
                <div className="flex items-center justify-between rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-5">
                  <div>
                    <p className="text-xs font-bold uppercase tracking-widest text-emerald-700">Negotiation successful</p>
                    <div className="mt-2 flex items-end gap-3">
                      <p className="font-mono text-3xl font-bold text-emerald-800">{money(negResult.negotiatedPrice)}</p>
                      <p className="mb-1 text-sm text-emerald-700 line-through opacity-70">{money(negResult.originalPrice)}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-[11px] font-bold uppercase tracking-wider text-emerald-700">Total savings</p>
                    <p className="mt-1 font-mono text-xl font-bold text-emerald-800">+{money(negResult.savings)}</p>
                  </div>
                </div>
              ) : negResult.thread ? (
                <div className="flex items-center gap-4 rounded-xl border border-amber-500/30 bg-amber-500/10 p-5">
                  <div className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-amber-500/20 text-amber-700"><Info size={18} /></div>
                  <div>
                    <p className="text-sm font-bold text-amber-800">No discount available</p>
                    <p className="mt-1 text-xs leading-5 text-amber-700/80">The vendor is currently unable to offer pricing below {money(negResult.originalPrice)}.</p>
                  </div>
                </div>
              ) : null}
              <div className="mt-5 flex justify-end">
                <button onClick={() => setNegResult(null)} className="rounded-lg bg-primary px-5 py-2.5 text-sm font-bold text-primary-foreground">Accept outcome</button>
              </div>
            </div>
          </div>
        </div>
